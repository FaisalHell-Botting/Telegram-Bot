    ...
    if old_msg_id:
        ...
        try:
            await context.bot.edit_message_text(
                ...
                text="🚫 تم إلغاء هذه العملية لبدء أمر جديد.",
                reply_markup=None
            )
        except Exception:
            pass
...
async def location_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        ...
        if editing_id:
            ...
            c.execute(
                "INSERT INTO orders (user_id, details, total_price, location, timestamp, status, is_paid) VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING id",
...
async def get_pay_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        c = conn.cursor()
        c.execute(
            "INSERT INTO orders (user_id, details, total_price, location, timestamp, status, is_paid) VALUES (%s, %s, %s, %s, %s, %s, %s)",
...
async def user_ledger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        c = conn.cursor()
        c.execute(
            "SELECT id, timestamp, total_price, details FROM orders WHERE user_id=%s AND is_paid=0 AND status='مقبول' ORDER BY id DESC",
            (user_id,)
        )
        rows = c.fetchall()
        c.close()
...
# ===== نظام التقييم =====
# ------------------------------------------------------------------
async def send_rating_request(bot, user_id: int, order_id: int):
    """إرسال طلب التقييم للزبون بعد تأكيد الطلب"""
    keyboard = [
...
async def handle_rating(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # التحقق: لو المستخدم ضغط على زر تقييم قديم
    ...
    with get_db() as conn:
        c = conn.cursor()
        # التحقق إذا سبق وقيّم هذا الطلب
        ...
        c.execute(
            "INSERT INTO ratings (user_id, order_id, rating, timestamp) VALUES (%s, %s, %s, %s)",
...
async def admin_ratings(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """عرض إحصائيات التقييمات للكاشير"""
...
async def cashier_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    with get_db() as conn:
        ...
        if data[0] == "conf":
            ...
            # ✅ إرسال طلب التقييم بعد تأكيد الطلب
            await send_rating_request(context.bot, user_id, order_id)
...
async def clear_debt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=user_id,
        text="✅ تم تأكيد استلام المبلغ وتصفير حسابك بنجاح. شكراً لك!"
    )
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(handle_rating, pattern="^rate_"))   # ✅ التقييمات
    app.add_handler(CommandHandler('ledger_admin', admin_ledger))
    app.add_handler(CommandHandler('ledger', user_ledger))
    app.add_handler(CommandHandler('ratings', admin_ratings))               # ✅ أمر التقييمات للكاشير
